...
def parse_filter_patterns(include: Optional[str], exclude: Optional[str]) -> dict:
    ...
    if exclude: raw_exclude.extend(exclude.split(","))
...
def should_exclude_file(file_path: Path, compiled: _CompiledPatterns) -> bool:
    ...
    if any(file_str_lower.endswith(ext) for ext in TOON_EXTENSIONS): return True
    if file_name in compiled.exact or any(part in compiled.exact for part in file_path.parts): return True
    if compiled.regex and (compiled.regex.search(file_name) or any(compiled.regex.search(part) for part in file_path.parts)): return True
    return False

def matches_include_pattern(file_path: Path, compiled: _CompiledPatterns) -> bool:
    if compiled.is_empty: return True
    file_name = file_path.name
    if file_name in compiled.exact: return True
