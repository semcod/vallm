...
class TestPipInstallation:
    ...
    def test_pip_install_from_local(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            project_root = Path(__file__).parent.parent
            subprocess.run(
                [str(pip_path), "wheel", str(project_root), "--wheel-dir", tmpdir], check=True
            )
...
class TestPipxInstallation:
    ...
    def test_pipx_install_editable(self):
        ...
        try:
            # Test installation
            result = subprocess.run(["vallm", "--help"], capture_output=True, text=True, check=True)
            assert "Validate LLM-generated code" in result.stdout

            result = subprocess.run(["vallm", "info"], capture_output=True, text=True, check=True)
            assert "vallm Configuration" in result.stdout
        finally:
            # Uninstall
    ...
    def test_pipx_install_with_extras(self):
        ...
        subprocess.run(
            ["pipx", "install", "--editable", f"{project_root}[llm,security]"], check=True
        )
...
class TestPostInstallation:
    ...
    def test_basic_functionality(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            project_root = Path(__file__).parent.parent
            subprocess.run(
                [str(python_path), "-m", "pip", "install", "-e", str(project_root)], check=True
            )
            ...
            result = subprocess.run(
                vallm_cmd + ["--help"], capture_output=True, text=True, check=True
            )
            assert "Validate LLM-generated code" in result.stdout

            result = subprocess.run(
                vallm_cmd + ["info"], capture_output=True, text=True, check=True
            )
            assert "vallm Configuration" in result.stdout
            ...
            result = subprocess.run(
                vallm_cmd + ["validate", "--file", str(test_file)], capture_output=True, text=True
            )
            ...
            result = subprocess.run(
                vallm_cmd + ["batch", str(test_project), "--recursive"],
                capture_output=True,
                text=True,
            )
            assert result.returncode in [0, 1, 2]
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for filename, expected_lang, code in test_cases:
                with tempfile.NamedTemporaryFile(mode="w", suffix=filename, delete=False) as f:
                    ...
                    result = subprocess.run(
                        vallm_cmd + ["validate", "--file", f.name], capture_output=True, text=True
                    )
