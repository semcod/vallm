...
if __name__ == "__main__":
    logging.basicConfig(
        ...
        handlers=[logging.StreamHandler(sys.stdout), logging.FileHandler("ollama_simple.log")],
    )
...
def analyze_with_code2llm(code_path: Path) -> Dict:
    ...
    try:
        ...
        return {"functions": len(functions), "classes": len(classes), "smells": 0, "source": code}
...
def validate_with_vallm(code: str) -> Dict:
    ...
    try:
        ...
        proposal = Proposal(code=code, language="python", filename="refactored.py")
...
"""
        ...
        # Check if we're done
        if validation["verdict"] == "pass" and test_result["success"] and overall_score >= 0.9:
            log_section("🎉 SUCCESS!")
            print(
                f"{Colors.GREEN}Perfect refactoring achieved in {iteration} iterations!{Colors.END}"
            )
            return {
                "success": True,
...
def main():
    ...
    parser.add_argument(
        "--file", type=str, default="legacy_code/simple_buggy.py", help="Path to legacy code file"
    )
    ...
    if result["success"]:
        ...
        print("  Best version: best_version.py")
    else:
    ...
    print("  - best_version.py")
    print("  - ollama_simple.log")
