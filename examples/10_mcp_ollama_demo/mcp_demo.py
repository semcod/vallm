...
if __name__ == "__main__":
    logging.basicConfig(
        ...
        handlers=[logging.StreamHandler(sys.stdout), logging.FileHandler("mcp_demo.log")],
    )
...
def analyze_with_code2llm(code_path: Path) -> dict:
    ...
    try:
        ...
        smells_info = [
            {"type": s.type, "message": s.message, "line": s.line} for s in project_result.smells
        ]
...
def run_mcp_workflow(code_path: Path, max_iterations: int = 3):
    ...
    while iteration <= max_iterations:
        ...
        if validation["verdict"] == "pass":
            ...
            print(
                f"{Colors.GREEN}Refactoring successful after {iteration} iteration(s){Colors.END}"
            )
...
"""
            ...
            print(
                f"{Colors.RED}Could not produce valid code after {max_iterations} attempts{Colors.END}"
            )
...
def main():
    ...
    parser.add_argument(
        "--file",
        type=str,
        default="legacy_code/order_processor.py",
        help="Path to legacy code file",
    )
    parser.add_argument(
        "--max-iterations", type=int, default=3, help="Maximum correction iterations"
    )
