...
from mcp.server._tools_vallm import validate_syntax, validate_security, validate_code
...
"""
    ...
    print(
        f"Summary: {result['summary']['total_issues']} issues, {result['summary']['error_count']} errors"
    )

    print("\nValidator results:")
    for validator_result in result["results"]:
        print(
            f"  {validator_result['validator']}: score {validator_result['score']}, {len(validator_result['issues'])} issues"
        )

    print("\nAll issues:")
    for issue in result["all_issues"]:
        print(
            f"  - [{issue['validator']}] {issue['message']} (line {issue['line']}, severity: {issue['severity']})"
        )
    print()
