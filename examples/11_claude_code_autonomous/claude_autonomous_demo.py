...
if __name__ == "__main__":
    logging.basicConfig(
        ...
        handlers=[logging.StreamHandler(sys.stdout), logging.FileHandler("claude_autonomous.log")],
    )
...
def analyze_with_code2llm(code_path: Path) -> Dict:
    ...
    try:
        ...
        functions_info = [
            {"name": f.name, "line": f.line_start, "complexity": getattr(f, "complexity", "N/A")}
            for f in project_result.functions
        ]
        classes_info = [
            {"name": c.name, "line": c.line_start, "methods": len(getattr(c, "methods", []))}
            for c in project_result.classes
        ]
        smells_info = [
            {
                "type": s.type,
            ...
            for s in project_result.smells
        ]
        coupling_info = [
            {"from_module": c.from_module, "to_module": c.to_module, "strength": c.strength}
            for c in project_result.coupling
        ]
...
def call_claude_code(
    prompt: str, model: str = "claude-3-5-sonnet-20241022", temperature: float = 0.1
) -> str:
...
def validate_with_vallm(code: str, description: str = "Code") -> Dict:
    ...
    proposal = Proposal(code=code, language="python", filename="refactored_code.py")
...
def run_runtime_tests(code_path: Path, test_file: Optional[Path] = None) -> Dict:
    ...
    try:
        ...
        print(
            f"{Colors.GREEN if success else Colors.RED}Tests {'PASSED' if success else 'FAILED'}{Colors.END}"
        )
        ...
        return {"success": False, "stdout": "", "stderr": str(e), "returncode": -1, "tests_run": 0}
...
def generate_claude_prompt(code: str, analysis: Dict, iteration: int = 1) -> str:
    ...
    if analysis.get("smells"):
        smells_text = "\n".join(
            [
                f"- [{s['type'].upper()}] Line {s['line']}: {s['message']}"
    ...
    if analysis.get("coupling"):
        coupling_text = "\n".join(
            [
                f"- {c['from_module']} → {c['to_module']} (strength: {c['strength']})"
...
def generate_feedback_prompt(
    current_code: str, validation: Dict, test_results: Dict, analysis: Dict
) -> str:
...
def run_autonomous_workflow(code_path: Path, max_iterations: int = MAX_5):
    ...
    log_section(
        "Claude Code Autonomous Demo: code2llm → Claude Code → vallm → Runtime Tests → code2llm"
    )
    ...
    while iteration <= max_iterations:
        ...
        if (
            ...
            and len(analysis.get("smells", [])) == 0
            and len(analysis.get("coupling", [])) == 0
        ):
            ...
            print("  Best version: best_refactored.py")
...
def main():
    ...
    parser.add_argument(
        "--file", type=str, default="legacy_code/data_processor.py", help="Path to legacy code file"
    )
    parser.add_argument(
        "--max-iterations", type=int, default=MAX_5, help="Maximum refactoring iterations"
    )
    ...
    print(
        f"{Colors.BOLD}Claude Code Autonomous: code2llm → Claude Code → vallm → Tests → code2llm{Colors.END}"
    )
    ...
    if result["success"]:
        ...
        print("  Output: best_refactored.py")
        print("  Log: claude_autonomous.log")
        sys.exit(0)
    else:
